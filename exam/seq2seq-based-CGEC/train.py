...
import torch
import transformers
from transformers import (AutoConfig, AutoModel, BertTokenizer,BertForTokenClassification,T5ForConditionalGeneration, T5Tokenizer, MT5Tokenizer, MT5ForConditionalGeneration,
...
custom_tokens = ["[EXP]", "[TYP]", "[NOG]"]
tokenizer.add_tokens(custom_tokens)
print("vocab(): ", tokenizer.get_vocab())
#tokenizer.add_special_tokens({"additional_special_tokens": ["[解释信息]", "[参考句]", "[错误类型]", "[待纠错句子]", "[纠正后的句子]"]})
...
type_column = 'error_type'
nogold_target_column = 'nogold_target'
...
def preprocess_function(examples):
    inputs = [
        f"correct grammar: {text} [NOG] {nogold} [TYP] {err_type} [EXP] {exp}" 
        for text, exp, err_type, nogold in zip(
            examples[text_column], 
            examples[explanation_column], 
            examples[type_column],
            examples[nogold_target_column]
        )
    ]
